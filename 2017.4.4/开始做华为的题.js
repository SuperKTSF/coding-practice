...
//sort排序本身就是按照ASCII码由小到大的顺序排序的
var readline = require("readline");
var rl = readline.createInterface(process.stdin, process.stdout); 
var args = [];
rl.on("line", function(newstr){
    args = newstr.trim().split("");
    var result = {};
    var count = [];
    var str="";
    for(var i = 0;i<args.length;i++){
    ...
    for(var item in result){
		if(count.indexOf(result[item])==-1){
			count.push(result[item]);
		}      
    }
    function compareTo(a,b){
        return b-a;
    }
    count.sort(compareTo);
    for(var j=0;j<count.length;j++){
		var sameCount = [];
        for(var item in result){
            if(result[item]==count[j]){
                sameCount.push(item);
            }
        }
		sameCount.sort();
		for(var k=0;k<sameCount.length;k++){
			str+=sameCount[k]
		}
    }
    console.log(str);     
...
rl.on("line", function(str){
    ...
    var pjun = (sum/(args.length-count)).toFixed(1);
    console.log(count);
    console.log(pjun);
});
//字符串的各种排序真的做的很多遍了
